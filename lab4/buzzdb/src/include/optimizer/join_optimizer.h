...
class PlanCache {
   ...
   public:
    /** Add a new cost, cardinality and ordering for a particular join set.  Does not verify that
       the new cost is less than any previously added cost -- simply adds or replaces an existing
       plan for the specified join set
    ...
    */
    ...
    void add_plan(std::set<LogicalJoinNode>& s, double cost, int card,
                  std::vector<LogicalJoinNode>& order) {
        best_orders[s] = order;
        best_costs[s] = cost;
    ...
    */
    double get_cost(std::set<LogicalJoinNode>& s) { return best_costs[s]; }
    ...
    */
    int get_card(std::set<LogicalJoinNode>& s) { return best_cardinalities[s]; }
};
